//////////////////////////////////////////////////////
//
//   Core+Beam Proton Cyclotron Anisotropy Instability
//
//////////////////////////////////////////////////////
...
begin_initialization {
  ...
  double n_core = 0.90;      // Core proton density fraction.
  double n_beam = 0.10;      // Beam proton density fraction.
  double Ub = -1.20*v_A;     // Beam drift along B0; here the beam moves in -x.
  double Uc = -n_beam*Ub/n_core; // Zero net ion current in the simulation frame.
  double beta_core_par = 0.80;
  double beta_beam_par = 0.40;
  double A_core = 1.20;      // T_perp/T_parallel for core protons.
  double A_beam = 2.50;      // T_perp/T_parallel for beam protons.
  double gamma = 5.0/3.0;   // Ratio of specific heats.
  double Lx_di = 128.0;      // Domain size in ion skin depth.
  ...
  double nppc  = 1024;          // Average number of macro particles per cell, all ions.
  ...
  sim_log ( "Uc/v_A = " << Uc/v_A );
  sim_log ( "Ub/v_A = " << Ub/v_A );
 ...
 // LOAD PARTICLES
  ...
  double zmin = grid->z0 , zmax = grid->z0+(grid->dz)*(grid->nz);
   ...
   repeat ( Ni_core/nproc() ) {
     ...
     ux = normal( rng(0), Uc, vth_core_par);
     uy = normal( rng(0), 0,  vth_core_perp);
     uz = normal( rng(0), 0,  vth_core_perp);
   ...
   repeat ( Ni_beam/nproc() ) {
     ...
     ux = normal( rng(0), Ub, vth_beam_par);
     uy = normal( rng(0), 0,  vth_beam_perp);
     uz = normal( rng(0), 0,  vth_beam_perp);
