...
program translate
  ...
  real(kind=4), allocatable, dimension(:,:,:) :: ex,ey,ez,bx,by,bz,jx,jy,jz,rho,ni_c,ux_c,uy_c,uz_c,pxx_c,pyy_c,pzz_c
  real(kind=4), allocatable, dimension(:,:,:) :: ni_b, ux_b, uy_b, uz_b, pxx_b, pyy_b, pzz_b, pxy_b, pxz_b, pyz_b
  real(kind=4), allocatable, dimension(:,:,:) :: pxy_c,pxz_c,pyz_c,phi, phit
...
! Allocate storage space for fields and moments
  ...
  allocate(ux_c(ht%nx,ht%ny,ht%nz))
  allocate(uy_c(ht%nx,ht%ny,ht%nz))
  allocate(uz_c(ht%nx,ht%ny,ht%nz))
  allocate(ni_c(ht%nx,ht%ny,ht%nz))
  allocate(pxx_c(ht%nx,ht%ny,ht%nz))
  allocate(pyy_c(ht%nx,ht%ny,ht%nz))
  allocate(pzz_c(ht%nx,ht%ny,ht%nz))
  allocate(pyz_c(ht%nx,ht%ny,ht%nz))
  allocate(pxz_c(ht%nx,ht%ny,ht%nz))
  allocate(pxy_c(ht%nx,ht%ny,ht%nz))

  allocate(ux_b(ht%nx,ht%ny,ht%nz))
  allocate(uy_b(ht%nx,ht%ny,ht%nz))
  allocate(uz_b(ht%nx,ht%ny,ht%nz))
  allocate(ni_b(ht%nx,ht%ny,ht%nz))
  allocate(pxx_b(ht%nx,ht%ny,ht%nz))
  allocate(pyy_b(ht%nx,ht%ny,ht%nz))
  allocate(pzz_b(ht%nx,ht%ny,ht%nz))
  allocate(pyz_b(ht%nx,ht%ny,ht%nz))
  allocate(pxz_b(ht%nx,ht%ny,ht%nz))
  allocate(pxy_b(ht%nx,ht%ny,ht%nz))
...
! for core ion hydro
              ...
              write(fname,"(A,I0,A,I0,A,I0)")"hydro/T.",tindex,"/Hhydro_c.",tindex,".",n-1        
              if (check) then 
                 open(unit=10,file=trim(fname),status='unknown',form='unformatted',access='stream')
              else
                 print *,"Can't find file:",fname
              ...
              read(10)buffer
              ux_c(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              uy_c(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              uz_c(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              ni_c(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)
              ...
              read(10)buffer
              pxx_c(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              pyy_c(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              pzz_c(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              pyz_c(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              pxz_c(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              pxy_c(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)
...
! for beam ion hydro
              ...
              write(fname,"(A,I0,A,I0,A,I0)")"hydro/T.",tindex,"/Hhydro_b.",tindex,".",n-1        
              if (check) then 
                 open(unit=10,file=trim(fname),status='unknown',form='unformatted',access='stream')
              else
                 print *,"Can't find file:",fname
              ...
              read(10)buffer
              ux_b(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              uy_b(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              uz_b(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              ni_b(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)
              ...
              read(10)buffer
              pxx_b(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              pyy_b(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              pzz_b(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              pyz_b(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              pxz_b(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)

              read(10)buffer
              pxy_b(idxstart(n,1):idxstop(n,1), idxstart(n,2):idxstop(n,2), idxstart(n,3):idxstop(n,3)) = &
                 buffer(2:nc(1)-1,2:nc(2)-1,2:nc(3)-1)
    ...
    where (abs(ni_c) > 0.0) 
        pxx_c = (pxx_c - mi_me*ux_c*ux_c/ni_c)
        pyy_c = (pyy_c - mi_me*uy_c*uy_c/ni_c)
        pzz_c = (pzz_c - mi_me*uz_c*uz_c/ni_c)
        pxy_c = (pxy_c - mi_me*ux_c*uy_c/ni_c)
        pxz_c = (pxz_c - mi_me*ux_c*uz_c/ni_c)
        pyz_c = (pyz_c - mi_me*uy_c*uz_c/ni_c)
        ux_c = (ux_c/ni_c)
        uy_c = (uy_c/ni_c)
        uz_c = (uz_c/ni_c)
     ...
     endwhere
     where (abs(ni_b) > 0.0) 
      pxx_b = (pxx_b - mi_me*ux_b*ux_b/ni_b)
      pyy_b = (pyy_b - mi_me*uy_b*uy_b/ni_b)
      pzz_b = (pzz_b - mi_me*uz_b*uz_b/ni_b)
      pxy_b = (pxy_b - mi_me*ux_b*uy_b/ni_b)
      pxz_b = (pxz_b - mi_me*ux_b*uz_b/ni_b)
      pyz_b = (pyz_b - mi_me*uy_b*uz_b/ni_b)
      ux_b = (ux_b/ni_b)
      uy_b = (uy_b/ni_b)
      uz_b = (uz_b/ni_b)
     elsewhere
    ...
    call write_data(fnames(1),ex,tindex,output_record)
     ...
     call write_data(fnames(7),ux_c,tindex,output_record)
     call write_data(fnames(8),uy_c,tindex,output_record)
     call write_data(fnames(9),uz_c,tindex,output_record)

     call write_data(fnames(10),ni_c,tindex,output_record)

     call write_data(fnames(11),pxx_c,tindex,output_record)
     call write_data(fnames(12),pyy_c,tindex,output_record)
     call write_data(fnames(13),pzz_c,tindex,output_record)
     call write_data(fnames(14),pyz_c,tindex,output_record)
     call write_data(fnames(15),pxz_c,tindex,output_record)
     call write_data(fnames(16),pxy_c,tindex,output_record)

     call write_data(fnames(17),ux_b,tindex,output_record)
     call write_data(fnames(18),uy_b,tindex,output_record)
     call write_data(fnames(19),uz_b,tindex,output_record)

     call write_data(fnames(20),ni_b,tindex,output_record)

     call write_data(fnames(21),pxx_b,tindex,output_record)
     call write_data(fnames(22),pyy_b,tindex,output_record)
     call write_data(fnames(23),pzz_b,tindex,output_record)
     call write_data(fnames(24),pyz_b,tindex,output_record)
     call write_data(fnames(25),pxz_b,tindex,output_record)
     call write_data(fnames(26),pxy_b,tindex,output_record)
