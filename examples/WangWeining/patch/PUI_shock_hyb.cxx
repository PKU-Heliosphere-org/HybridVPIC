...
double speed_pdf(double x, double r, double alpha, double eta) {  
    ...
    if (x <= 0) return 0.0;  
    return pow(x,alpha-1)*exp(-lambda/r*pow(x,-alpha))*stepFunction(1-x) ; // 
}  
...
begin_initialization {
  ...
  double beta_i   = 0.04;     //1.0         // Background ion beta
  ...
  double Npui  = nppc*nx*ny*nz/3;         // Total macroparticle PUIs in box
  ...
  int Hparticle_interval = interval/20;
...
# define RANK_TO_INDEX(rank,ix,iy,iz) BEGIN_PRIMITIVE {                 \
  ...
  global->q[1]  = qi;
  global->npleft[0]  = n0;
  global->npleft[1]  = n0/3;
  global->npright[0]  = n0;
  global->npright[1]  = n0/3;
   ...
   */
   ...
   repeat ( Npui/nproc() ) {
    ...
    // ux_swi = normal( rng(0), 0, vthi)-Vd;//inverse_cdf(uniform(rng(0),0,1), 0.5, 1e-2, 1000)-Vd;//normal( rng(0), 0, vthi)-Vd;
     ...
     theta_pui = uniform(rng(0),0,M_PI);
     phi_pui = uniform(rng(0),0,2*M_PI);
     V = Vd*inverse_cdf(uniform(rng(0),0,1), 1e-3);
	...
	for ( int k=1;k<=nz; k++ ) {
      ...
      //std::cout<<species->np<<"\n";
        ...
        //end if
          ...
          double ux_pui, uy_pui, uz_pui, V;
          double theta_pui, phi_pui;
          theta_pui = uniform(rng(0),0,M_PI);
          phi_pui = uniform(rng(0),0,2*M_PI);

          V = global->ur*inverse_cdf(uniform(rng(0),0,1), 1e-3);
