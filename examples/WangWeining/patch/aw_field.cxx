...
if (global->right) {
  ...
  XYZ_LOOP(nx-1,nx,0,ny+1,0,nz+1) field(x,y,z).cbx = b0 * sqrt(1-sn*sn);//0;
  XYZ_LOOP(nx-1,nx,0,ny+1,0,nz+1) field(x,y,z).cby = b_amp * phase_factor(x, y, z);
  XYZ_LOOP(nx-1,nx,0,ny+1,0,nz+1) field(x,y,z).cbz = b0 * sn;// b0 // Do not forget the background value.
