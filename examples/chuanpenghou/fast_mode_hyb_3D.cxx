...
#include "fast_mode_polarization_low_beta_3D.cxx"
//////////////////////////////////////////////////////
...
begin_initialization {
  ...
  // double Cs = sqrt(gamma*beta_i/2)*v_A;  // reference: Shestov et al., A&A 2022 https://doi.org/10.1051/0004-6361/202142362

  // Numerical parameters
  // double taui = 500;    // Simulation run time in wci^-1.
 
  double Lx    = 1024*di;    // size of box in x dimension
  double Ly    = 1024*di;    // size of box in y dimension
  ...
  // // ***** Set Particle Boundary Conditions *****
  // if ( iy==0 )    set_domain_particle_bc( BOUNDARY(0,-1,0), reflect_particles );
  // if ( iy==topology_y-1 ) set_domain_particle_bc( BOUNDARY(0,1,0), reflect_particles );
 ...
 // Dump simulation information to file "info.bin" for translate script
  ...
  const int re_num = 1;
  double kxmin = -3; double kxmax = 3;  //floor(2.0*M_PI/4.0/hx/kx0);
  double kymin = -3; double kymax = 3;  //floor(2.0*M_PI/4.0/hy/ky0);
  double kzmin = -3; double kzmax = 3;  //floor(2.0*M_PI/4.0/hz/kz0);
...
PerturbationParams params = {amplitude_ratio, kx_random, ky_random, kz_random, phi_random, n_modes, n0, Cs, global->v_A, b0x, b0y, b0z, waveamp};
...
sim_log( "-> non-uniform plasma + specified waves" );
...
repeat (static_cast<int>(std::floor(nlocal))) {
  ...
  } while(r > (n0 + N_PERT(x,y,z,params)));
...
begin_diagnostics {
  ...
  *------------------------------------------------------------------------*/
  ...
  if ( should_dump(Hparticle) && rank() == 0 ) {
...
//     for( y=yl; y<=yh; y++ )                     \
//       for( x=xl; x<=xh; x++ )
    // XYZ_LOOP(0,nx+1,0,ny+1,0,nz+1) field(x,y,z).ey = 0;
    // XYZ_LOOP(0,nx+1,0,ny+1,0,nz+1) field(x,y,z).ez = 0;
    // XYZ_LOOP(0,nx+1,0,ny+1,0,nz+1) field(x,y,z).cbz = 0;
}  // end field injection
