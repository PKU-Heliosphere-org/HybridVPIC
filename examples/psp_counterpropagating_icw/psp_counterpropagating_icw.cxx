//////////////////////////////////////////////////////
//
//   PSP Counterpropagating Ion-Cyclotron Waves
//
//////////////////////////////////////////////////////
...
begin_initialization {
  ...
  // PSP 2023-06-22 event parameters from the lecture notes.
  // Physical inputs: npc=2530 cm^-3, npb=2169 cm^-3, B0=780 nT,
  // Vsw=319.9 km/s, vpc=234 km/s, vpb=420 km/s. With these values
  // v_A=248.19 km/s, so drifts are normalized by v_A below.
  double n_core = 2530.0/(2530.0 + 2169.0);
  double n_beam = 2169.0/(2530.0 + 2169.0);
  double Uc = (234.0 - 319.9)/248.19215828208905*v_A;
  double Ub = (420.0 - 319.9)/248.19215828208905*v_A;
  // Parallel betas computed from the physical values in the notes.
  double beta_core_par = 0.03348972372942244;
  double beta_beam_par = 0.06890676120390571;
  // T_perp is multiplied by 1.4, following the simulation setup in the notes.
  double A_core = 52.0*1.4/20.0;  // T_perp/T_parallel for core protons.
  double A_beam = 107.0*1.4/48.0; // T_perp/T_parallel for beam protons.
  double gamma = 5.0/3.0;   // Ratio of specific heats.
  double Lz_di = 160.0;      // Domain size in ion skin depth.
  double Tipar_Te = 1.0;     // Electron temperature tied to total parallel ion pressure.

  double eta = 1.0e-3;      // Small normalized resistivity for high-k damping.
  double hypereta = 0.0; // Plasma hyper-resistivity.

  
  // Derived quantities for model:
  double Tcore_par = beta_core_par*b0*b0/2.0/n_core;
  double Tbeam_par = beta_beam_par*b0*b0/2.0/n_beam;
  double vth_core_par = sqrt(Tcore_par/mi);
  double vth_core_perp = vth_core_par*sqrt(A_core);
  double vth_beam_par = sqrt(Tbeam_par/mi);
  double vth_beam_perp = vth_beam_par*sqrt(A_beam);
  double Te = (n_core*Tcore_par + n_beam*Tbeam_par)/n0/Tipar_Te;
 

  // Numerical parameters
  double taui    = 700;   // Simulation run time in wci^-1.
  double quota   = 23.5;   // run quota in hours
  double quota_sec = quota*3600;  // Run quota in seconds
  
  double Lx    = 1.0*di;   // size of box in x dimension
  double Ly    = 1.0*di;   // size of box in y dimension
  double Lz    = Lz_di*di; // size of box in z dimension

  double nx = 1;
  double ny = 1;
  double nz = 512;

  double nppc  = 2048;          // Practical default. Increase toward 2^18 for production.
  ...
  double Ni_total = nppc*nx*ny*nz;
  double Ni_core  = trunc_granular(Ni_total*n_core/n0,nproc());
  double Ni_beam  = trunc_granular(Ni_total*n_beam/n0,nproc());
  double Np_core  = n_core*Lx*Ly*Lz;
  double Np_beam  = n_beam*Lx*Ly*Lz;
  double qi_core = ec*Np_core/Ni_core;
  double qi_beam = ec*Np_beam/Ni_beam;

  
  // Determine the time step
  double dg = courant_length(Lx,Ly,Lz,nx,ny,nz);  // courant length
  double dt = 0.05/wci;                           // time step from the lecture notes.
  ...
  int Hparticle_interval = int(500.0/(wci*dt)); // Sparse VDF snapshots every 500 Omega_ci^-1.
  ...
  double nmax_c = 1.5*Ni_core/nproc();
  double nmax_b = 1.5*Ni_beam/nproc();
  double nmovers_c = 0.1*nmax_c;
  double nmovers_b = 0.1*nmax_b;
  double sort_method = 1;   // 0=in place and 1=out of place
  species_t *ion_c = define_species("ion_c", ec, mi, nmax_c, nmovers_c, sort_interval, sort_method);
  species_t *ion_b = define_species("ion_b", ec, mi, nmax_b, nmovers_b, sort_interval, sort_method);
  ...
  sim_log ( "case = PSP 2023-06-22 counterpropagating ICWs" );
  sim_log ( "Lz_di = " << Lz_di );
  sim_log ( "Tipar/Te = " << Tipar_Te ) ;
  sim_log ( "n_core = " << n_core );
  sim_log ( "n_beam = " << n_beam );
  sim_log ( "Uc/v_A = " << Uc/v_A << " (core, plasma frame)" );
  sim_log ( "Ub/v_A = " << Ub/v_A << " (beam, plasma frame)" );
  sim_log ( "beta_core_par = " << beta_core_par );
  sim_log ( "beta_beam_par = " << beta_beam_par );
  sim_log ( "A_core = " << A_core );
  sim_log ( "A_beam = " << A_beam );
  sim_log ( "taui = " << taui );
  ...
  sim_log ( "v_A = " << v_A );
  sim_log ( "di = " << di );
  sim_log ( "Ni_core = " << Ni_core );
  sim_log ( "Ni_beam = " << Ni_beam );
  sim_log ( "total # of particles = " << Ni_core + Ni_beam );
  sim_log ( "dt*wci = " << wci*dt );
  sim_log ( "energies_interval: " << energies_interval << " (0 disables ASCII energy dump on local macOS runs)" );
  sim_log ( "dx/di = " << Lx/(di*nx) );
  sim_log ( "dy/di = " << Ly/(di*ny) );
  sim_log ( "dz/di = " << Lz/(di*nz) );
  sim_log ( "n0 = " << n0 );
  sim_log ( "vth_core_par/v_A = " << vth_core_par/v_A );
  sim_log ( "vth_beam_par/v_A = " << vth_beam_par/v_A );
...
// In general, regions are specied as logical equations (i.e. x>0 && x+y<2) 
  set_region_field( everywhere, 0, 0, 0, 0, 0, 1.0);
 ...
 // LOAD PARTICLES
  ...
  double zmin = grid->z0 , zmax = grid->z0+(grid->dz)*(grid->nz);
   ...
   repeat ( Ni_core/nproc() ) {
     ...
     ux = normal( rng(0), 0,  vth_core_perp);
     uy = normal( rng(0), 0,  vth_core_perp);
     uz = normal( rng(0), Uc, vth_core_par);

     inject_particle( ion_c, x, y, z, ux, uy, uz, qi_core, 0, 0);

   }

   repeat ( Ni_beam/nproc() ) {
     double x, y, z, ux, uy, uz, d0 ;
     ...
     ux = normal( rng(0), 0,  vth_beam_perp);
     uy = normal( rng(0), 0,  vth_beam_perp);
     uz = normal( rng(0), Ub, vth_beam_par);

     inject_particle( ion_b, x, y, z, ux, uy, uz, qi_beam, 0, 0);
  ...
  sim_log ( "Core/beam ion species output format = band" );
  ...
  //sprintf(global->hHdParams_c.baseDir, "hydro/%d",NUMFOLD);
  dump_mkdir("hydro");
  dump_mkdir(global->hHdParams_c.baseDir);

  //// base file name for fields output
  //sprintf(global->hHdParams_c.baseFileName, "Hhydro");
...
// global->hHdParams_c.output_variables( all );
  ...
  sim_log ( "Core ion species variable list: " << varlist );
  create_hydro_list(varlist, global->hHdParams_b);
  sim_log ( "Beam ion species variable list: " << varlist );
...
#define should_dump(x)                                                  \
  (global->x##_interval>0 && (step()%global->x##_interval) == 0)
...
begin_diagnostics {
  ...
  if(should_dump(Hhydro)) {
    hydro_dump("ion_c", global->hHdParams_c);
  ...
  *------------------------------------------------------------------------*/
  ...
  // Dump sparse particle snapshots for VDF diagnostics.
  // These files are large. They are intentionally infrequent: t=0, 500 Omega_ci^-1,
  // and the final time.

  char particle_time_dir[64];
  char subdir_c[64];
  char subdir_b[64];

  if ( step()==0 || should_dump(Hparticle) || step()==num_step ) {
    sprintf(particle_time_dir,"particle/T.%lld",(long long)step());
    dump_mkdir(particle_time_dir);
    sprintf(subdir_c,"particle/T.%lld/ion_c",(long long)step());
    sprintf(subdir_b,"particle/T.%lld/ion_b",(long long)step());
    sim_log("Dumping particle snapshots");
    dump_particles("ion_c", subdir_c);
    dump_particles("ion_b", subdir_b);
    sim_log("Finished particle snapshots");
  }
